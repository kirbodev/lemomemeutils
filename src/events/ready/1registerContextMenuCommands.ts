...
export default async (client: Client) => {
    try {
        const localCommands = (await getLocalCommands()).filter(cmd => cmd.contextMenu !== undefined);
        const commands = (await client.application?.commands.fetch())?.filter(cmd => cmd.type === ApplicationCommandType.User);
        if (!commands) {
        ...
        for (const localCommand of localCommands) {
            ...
            const existingCommand = commands.find((command) => command.name === contextName ?? name);
