import { ActionRowBuilder, ButtonBuilder, ButtonStyle, Client, EmbedBuilder, GuildMemberRoleManager, Interaction, ModalActionRowComponentBuilder, ModalBuilder, PermissionsBitField, TextChannel, TextInputBuilder, TextInputStyle } from "discord.js";
...
export default async (client: Client, interaction: Interaction) => {
    ...
    if (config.staffAppRoleID && !(interaction.member?.roles as GuildMemberRoleManager).cache.has(config.staffAppRoleID)) return interaction.reply({
        embeds: [
            new EmbedBuilder()
                .setTitle(Errors.ErrorUser)
                .setDescription(`You do not have permission to apply for staff. You need <@&${config.staffAppRoleID}> to apply for staff.`)
                .setColor(EmbedColors.error)
                .setFooter({
                    text: `Requested by ${interaction.user.tag}`,
                    iconURL: interaction.user.displayAvatarURL()
    ...
    try {
        ...
        if (!age || !timezone || !level || !why || !mfa) return response.reply({
            embeds: [
                new EmbedBuilder()
                    .setTitle(Errors.ErrorUser)
        ...
        if (mfa.toLowerCase() !== "y" && mfa.toLowerCase() !== "n") return response.reply({
            embeds: [
                new EmbedBuilder()
                    .setTitle(Errors.ErrorUser)
                    .setDescription('You did not fill out the form correctly. 2fa must be y (yes) or n (no).')
                    .setColor(EmbedColors.error)
                    .setFooter({
                        text: `Requested by ${interaction.user.tag}`,
                        iconURL: interaction.user.displayAvatarURL()
        ...
        if (mfa.toLowerCase() === "n") return response.reply({
            embeds: [
                new EmbedBuilder()
                    .setTitle(Errors.ErrorUser)
                    ...
                    .setFields([
                        {
                            ...
                            value: "Check out [this article](https://support.discord.com/hc/en-us/articles/219576828-Setting-up-Multi-Factor-Authentication)."
                        }
                    ])
                    .setColor(EmbedColors.error)
                    .setFooter({
