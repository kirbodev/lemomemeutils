...
export default async (client: Client, reaction: MessageReaction, user: User) => {
    ...
    const staffApp: HydratedDocument<staffInterface> | null = await Staff.findOne({ voteMessage: reaction.message.id, guildID: reaction.message.guild.id, "decision.decisionAt": undefined });
    if (!staffApp) return;
    const voteType = reaction.emoji.name === "✅";
    const vote = staffApp.decision.votes.get(user.id);
    if (vote !== undefined && vote === voteType) {
        await reaction.message.reactions.cache.get(voteType ? "❌" : "✅")?.users.remove(user.id);
    }
