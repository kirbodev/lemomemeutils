...
export default {
  ...
  execute: async (client: Client) => {
    ...
    const ebans = bans.filter(
      (ban) => ban.expiresAt && ban.expiresAt.getTime() < Date.now(),
    );
    ...
    for (const user of users) {
      ...
      const mostRecentBan = userBans.sort(
        (a, b) => b.timestamp!.getTime() - a.timestamp!.getTime(),
      )[0];
      if (!mostRecentBan) continue;
      if (
        mostRecentBan.expiresAt &&
        mostRecentBan.expiresAt.getTime() > Date.now()
      )
        continue;
      ...
      await client.guilds.cache
        .get(mostRecentBan.guildID)
        ?.members.unban(user, "Ban expired");
      count++;
    }
