...
export default {
  ...
  execute: async (client: Client) => {
    let count = 0;
    const ices: HydratedDocument<actionInterface>[] = await Action.find({
      iceSeverity: { $ne: null },
    });
      ...
      for (const guild of userIces.map((ice) => ice.guildID)) {
        ...
        for (const ice of userIces) {
          if (
            ice.iceSeverity == 0 &&
            Date.now() <
              (ice.timestamp as Date).getTime() + 1000 * 60 * 60 * 24 * 14
          )
            thinIce = true;
          if (
            ice.iceSeverity == 1 &&
            Date.now() <
              (ice.timestamp as Date).getTime() + 1000 * 60 * 60 * 24 * 30
          )
            thinnerIce = true;
        }
        ...
        const thinnerIceRole = member?.roles.cache.get(
