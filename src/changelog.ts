export default {
    name: "Bug Fixes + QOL Changes",
    date: new Date(2024, 0, 10),
    version: "1.3.0",
    description: "Fixed a couple of bugs and added some QOL changes. Added message commands.",
    changelog: [
        "Added message commands. Not as stable as slash commands, but they work.",
        "Added changelog command.",
        "Bot no longer sends the same message twice when using commands in the log channel.",
        "Apples can now use the ban button from having too many warnings.",
        "Help command now displays the correct cooldown.",
    ]
}