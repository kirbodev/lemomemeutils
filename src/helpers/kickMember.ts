    ...
    await member.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `You have been kicked from \`${mod.guild.name}\`. You can rejoin at any time, but if you continue to break the rules, you will be banned. ${invite ? `discord.gg/${invite}` : ""}`,
          )
          .setFields([
            {
              name: "Reason",
            ...
            },
            {
              ...
              value: warns
                .map(
                  (warn) =>
                    `<t:${Math.floor(warn.timestamp.getTime() / 1000)}:f> - ${warn.reason} - Issued by <@${warn.moderatorID}>`,
                )
                .join("\n"),
            },
          ])
