...
export default {
                ...
                return interaction.followUp({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(`Something went wrong while banning <@${user.id}>.`)
                            .setColor(EmbedColors.error)
                            .setFooter({
                                text: `Requested by ${interaction.user.tag}`,
                                iconURL: interaction.user.displayAvatarURL()
    ...
    message: async (interaction: Message, { args, alias }) => {
        ...
        let parole = !time && args[1] && (args[1].toLowerCase() === "true" || args[1].toLowerCase() === "false") ? args[1].toLowerCase() === "true" : undefined;
        // Check if args[2] is a parole
        parole = time && args[2] && (args[2].toLowerCase() === "true" || args[2].toLowerCase() === "false") ? args[2].toLowerCase() === "true" : parole;
        ...
        if (alias === "fban") {
            const embed = new EmbedBuilder()
                .setTitle("Banned")
                .setDescription(`Banned <@${user.id}> for \`${reason ?? "No reason provided"}\`. They have been notified.`)
                .setFields([
                    {
                        name: "Parole",
                        value: `${parole ? "Yes" : "No"}`,
