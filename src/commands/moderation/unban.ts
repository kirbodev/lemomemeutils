import type { ChatInputCommandInteraction, Message, /* ContextMenuCommandInteraction, Message */ } from "discord.js";
...
export default {
    ...
    message: async (interaction: Message, { args }) => {
        args = args ?? [];
        const userID = args[0];
        const parole = args[1] === "true" || args[2] === "true" ? true : args[1] === "false" || args[2] === "false" ? false : undefined;
        const ice = args[1] === "thin" || args[2] === "thin" ? "thin" : args[1] === "thinner" || args[2] === "thinner" ? "thinner" : undefined;
        const reason = parole ? ice ? args.slice(3).join(" ") : args.slice(2).join(" ") : args.slice(1).join(" ");
        const config = configs.get(interaction.guildId!)!;
